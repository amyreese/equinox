...
VS_OUTPUT VertexShaderFunction( VS_INPUT input )
{
    ...
    output.lightDirection = DiffuseLight1Pos - worldSpacePos;
...
float4 PixelShaderFunction( VS_OUTPUT input ) : COLOR0
{
    ...
    float4 diffuse = (DiffuseLight1Color * DiffuseLight1Intensity) * nDotL;
    ...
    float4 specular = Shininess * DiffuseLight1Color * pow(rDotV, SpecularPower);
    
    float4 diffuseTexture = tex2D(DiffuseTextureSampler, input.texCoord);
    
    // return the combined result.
    return (diffuse + (AmbientLightColor * AmbientLightIntensity ) ) * diffuseTexture + specular;
}
